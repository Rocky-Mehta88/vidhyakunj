...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		protected $site_style_theme_mod_key = 'skilled_site_style'; 
		...
		public function init_globals() {
			...
			$config = include get_template_directory() . '/lib/integrations/envato_setup/config.php';

			$current_theme         = wp_get_theme();
			$this->envato_username = $config['envato_username'];
			$this->oauth_script    = $config['oauth_script'];
		...
		public function redux_panel_will_appear_here() {
			...
			<div class="notice notice-warning"> 
				<p><strong><?php esc_html_e('Redux panel will appear here.', 'skilled'); ?></strong></p>
			</div>
			<?php
		}
		...
		public function admin_menus() {
			if ( ! class_exists( 'Redux' ) ) {
				...
				add_menu_page(
			        esc_html__( 'Theme Options', 'skilled' ),
			        esc_html__( 'Theme Options', 'skilled' ),
			        'manage_options',
			        $this->parent_slug,
			        array(
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'skilled' ), esc_html__( 'Setup Wizard', 'skilled' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'skilled' ), esc_html__( 'Setup Wizard', 'skilled' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'skilled' ),
					'view'    => array( $this, 'envato_setup_introduction' ),
					'handler' => array( $this, 'envato_setup_introduction_save' ),
				),
			);
			$this->steps['requirements'] = array(
				'name'    => esc_html__( 'Requirements', 'skilled' ),
				'view'    => array( $this, 'envato_setup_requirements' ),
				'handler' => '',
			);
			if ( class_exists( 'TGM_Plugin_Activation' ) && isset( $GLOBALS['tgmpa'] ) ) {
				$this->steps['default_plugins'] = array(
					'name'    => esc_html__( 'Plugins', 'skilled' ),
					'view'    => array( $this, 'envato_setup_default_plugins' ),
					'handler' => '',
				);
			}
			$this->steps['updates'] = array(
				'name'    => esc_html__( 'Updates', 'skilled' ),
				'view'    => array( $this, 'envato_setup_updates' ),
				'handler' => array( $this, 'envato_setup_updates_save' ),
			);
			$this->steps['upload_demo'] = array(
				'name'    => esc_html__( 'Demo', 'skilled' ),
				'view'    => array( $this, 'envato_setup_upload_demo' ),
				'handler' => array( $this, 'envato_setup_upload_demo_save' ),
			);
			$this->steps['default_content'] = array(
				'name'    => esc_html__( 'Content', 'skilled' ),
				'view'    => array( $this, 'envato_setup_default_content' ),
				'handler' => '',
			);
			if ($this->get_setting('use_layer_slider') || $this->get_setting('use_rev_slider')) {
				$this->steps['slider']      = array(
					'name'    => esc_html__( 'Slider', 'skilled' ),
					'view'    => array( $this, 'envato_setup_rev_slider_handler' ),
					'handler' => '',
				);
			}
			if ($this->get_setting('use_ultimate_icons')) {

				$this->steps['ultimate_icons']      = array(
					'name'    => esc_html__( 'Icons', 'skilled' ),
					'view'    => array( $this, 'envato_setup_ultimate_icons' ),
					'handler' => array( $this, 'envato_setup_ultimate_icons_handler' ),
				);
			}
			$this->steps['customize']       = array(
				'name'    => esc_html__( 'Customize', 'skilled' ),
				'view'    => array( $this, 'envato_setup_customize' ),
				'handler' => '',
			);
			$this->steps['help_support']    = array(
				'name'    => esc_html__( 'Support', 'skilled' ),
				'view'    => array( $this, 'envato_setup_help_support' ),
				'handler' => '',
			);
			$this->steps['next_steps']      = array(
				'name'    => esc_html__( 'Ready!', 'skilled' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'skilled' ),
			) );
			...
			// Ultimate Addons is not checking if current screen is set before trying to get prop value.
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			echo '<t';
			echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'skilled' ) . '</ti' . 'tle>'; ?>
			...
			<a href="http://themeforest.net/user/aislin/portfolio" target="_blank"><?php
				...
				if ( $image_url ) {
					...
					<img src="<?php echo esc_url( $this->plugin_url . 'images/logo.png' ); ?>"
					     alt="<?php esc_attr_e( 'Envato install wizard', 'skilled' ); ?>" /><?php
				} ?></a>
		</h1>
		<?php
		}
		...
		<a class="wc-return-to-dashboard"
		   href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'skilled' ); ?></a>
		</body>
		<?php
	...
	<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'skilled' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'skilled' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'skilled' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'skilled' ); ?></a>
				</p>
			<?php
			} else if ( get_option( 'envato_setup_complete', false ) ) {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'skilled' ), wp_get_theme() ); ?></h1>
				<p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ', 'skilled' ); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'skilled' ); ?></a>
					</li>
				</ul>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'skilled' ); ?></a>
				</p>
			<?php
			} else {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'skilled' ), wp_get_theme() ); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. It should only take 5 minutes.', 'skilled' ), wp_get_theme() ); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'skilled' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'skilled' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'skilled' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function envato_setup_requirements() {
			...
			?>
			<h3><?php esc_html_e( 'Befeore proceeding, please watch installation video and read the requirements bellow', 'skilled' ); ?></h3>
			<p>
				<a target="_blank" href="https://www.youtube.com/watch?v=EJejgZ5YQNk&t=6s"><img
					src="<?php echo esc_url( get_template_directory_uri() . '/assets/img/theme-install-video.jpg' ); ?>"
					alt="<?php esc_attr_e( 'Installation video', 'skilled' ); ?>"/></a>
			</p>
			<h2><?php esc_html_e( 'Important!', 'skilled' ); ?></h2>
			<p><strong><?php esc_html_e( 'Please note that Sensei plugin is not included in the theme and needs to be purchased separately. A free version of the plugin is available. Please follow the link bellow.',
						'skilled' ); ?></strong>
				<?php esc_html_e( 'If you choose to use it you need to install it before you proceed to importing demo content.',
						'skilled' ); ?></p>
				<?php printf( esc_attr( 'Please follow this %s. Also, please read the documentation on the plugin.', 'skilled' ), '<a href="http://www.woothemes.com/products/sensei/" target="_blank">link</a>'); ?></p>
				<a class="button-primary button" href="<?php echo esc_url( admin_url( 'plugin-install.php?tab=upload' ) ); ?>"><?php esc_html_e( 'Install plugin', 'skilled' ); ?></a>
			<h4><?php printf( esc_attr( 'To use %s Theme, you must be running:', 'skilled' ), $theme_name ); ?></h4>
			<ul>
			...
			<h4>
				<?php esc_html_e( 'Many issues that you may run into such as: white screen, demo content fails when importing and other similar issues are all related to low PHP configuration limits. The solution is to increase the PHP limits. You can do this on your own, or contact your web host and ask them to increase those limits to a minimum as follows:', 'skilled' ); ?>
			</h4>
			<ul>
			...
			<ul>
				...
				<li><?php printf( esc_attr( 'You can download the latest release of WordPress from official %s website.', 'skilled' ), '<a href="https://wordpress.org/" target="_blank">WordPress</a>' ); ?></li>
				<li><?php esc_html_e( 'Always create secure passwords for FTP and Database.', 'skilled' ); ?></li>
			</ul>
			<?php esc_html_e( 'Visual Composer and Revolution Slider plugins are included with the theme but you do not get purchase codes for these plugins.', 'skilled' ); ?>
			</h4>
			<p>
		...
		<?php if ( class_exists( 'WooCommerce' ) ) : ?>
				<br/>
				<p>
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-status' ) ); ?>"><?php esc_html_e( 'Check System Status page', 'skilled' ); ?></a>
				</p>
			<?php endif; ?>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button-primary button button-large button-next"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
			</p>
			<?php
		}
		...
		private function _get_plugins() {
			if ( function_exists( 'skilled_setup_wizard_get_plugins' ) ) {
				return skilled_setup_wizard_get_plugins();
			} else {
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'skilled' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'skilled' ); ?></p>
				...
				<?php
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'skilled' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'skilled' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'skilled' ); ?></a>
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'skilled' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'skilled' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'skilled' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'skilled' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'skilled' ) ) );
			}
			exit;
		...
		public function envato_setup_rev_slider() {
			...
			$is_rev_slider = $this->get_setting('use_rev_slider');
			$is_layer_slider = $this->get_setting('use_layer_slider');

			if ( ! ($is_rev_slider || $is_layer_slider) ) {
			...
			if ($is_rev_slider) {
				...
				$rev_importer = new Skilled_Revolution_Slider_Importer();
				$rev_importer->cleanup();
			} elseif ($is_layer_slider) {
				...
				if (class_exists('ZipArchive')) {
					...
					$ls_importer = new Skilled_Layer_Slider_Importer();
					$ls_importer->cleanup();
				} else {
					$error = esc_html__('Layer Slider requires PHP ZipArchive extension .zip files. Please contact your hosting provider to enable this for you', 'skilled');
				}
			}
			?>
			...
			<form method="post" enctype="multipart/form-data">
				...
				<?php else: ?>
					<p><?php echo sprintf(esc_html( 'Please select %s slider to import', 'skilled' ), $slider_name); ?></p>
				...
				<p class="envato-setup-actions step">
					<?php if (!$error): ?>
						<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'skilled' ); ?>" name="save_step"/>
					<?php endif ?>
		...
		public function envato_setup_rev_slider_handler() {
			...
			if ( $is_layer_slider ) {
				require_once( get_template_directory() . '/lib/integrations/demo-importer/includes/file-importer.php' );
				$ls_importer = new Skilled_File_Importer();
				$sliders = $ls_importer->read_zip_files_from_temp_path('layer-slider');
			}
			?>
			<form method="post">
				<p><?php esc_html_e( 'Please select which sliders to import', 'skilled' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_rev_slider"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'skilled' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_rev_slider_cleanup() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['cleanup'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'Error', 'skilled' ) ) );
			}
		...
		public function ajax_rev_slider() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slider'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slider Found', 'skilled' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'skilled' ),
				) );
			}
			exit;
		}

		public function envato_setup_upload_demo() {
			?>
			<form method="post" enctype="multipart/form-data">
				<p><?php esc_html_e( 'Please select demo zip file to import', 'skilled' ); ?></p>
				<input type="file" name="import_file">
				<p class="envato-setup-actions step">
						<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'skilled' ); ?>" name="save_step"/>
		...
		public function envato_setup_upload_demo_save() {
			check_admin_referer( 'envato-setup' );
			...
			require_once( get_template_directory() . '/lib/integrations/demo-importer/includes/file-importer.php' );
			$importer = new Skilled_File_Importer();
			$importer->cleanup();

			$filepath = $_FILES["import_file"]["tmp_name"];
			if ($filepath) {

				$success = $importer->upload_zip($filepath);
			}
		...
		public function get_setting($key) {
			...
			$config = $this->_get_json('config.json');
		...
		public function envato_setup_ultimate_icons() {
			...
			?>
			<form method="post">
				<p><?php esc_html_e( 'This demo uses Custom Icons, press Continue to install them.', 'skilled' ); ?></p>
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next"
					       value="<?php esc_attr_e( 'Continue', 'skilled' ); ?>" name="save_step"/>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
		...
		public function envato_setup_ultimate_icons_handler() {
			...
			require_once( get_template_directory() . '/lib/integrations/demo-importer/includes/file-importer.php' );
			require_once( get_template_directory() . '/lib/integrations/demo-importer/includes/ultimate-icons-importer.php' );
			$icons_importer = new Skilled_Ultimate_Icons_Importer();
			$success        = $icons_importer->copy_folder();
			if ($success) {
				$icons_importer->import_db_settings();
			}
		...
		private function _content_default_get() {
			...
			foreach ( $available_content as $post_type => $post_data ) {
				if ( count( $post_data ) ) {
					...
					$content[ $post_type ] = array(
						...
						'description'      => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'skilled' ), $post_type_title ),
						'pending'          => esc_html__( 'Pending.', 'skilled' ),
						'installing'       => esc_html__( 'Installing.', 'skilled' ),
						'success'          => esc_html__( 'Success.', 'skilled' ),
			...
			$content['widgets']  = array(
				'title'            => esc_html__( 'Widgets', 'skilled' ),
				'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'skilled' ),
				'pending'          => esc_html__( 'Pending.', 'skilled' ),
				'installing'       => esc_html__( 'Installing Default Widgets.', 'skilled' ),
				'success'          => esc_html__( 'Success.', 'skilled' ),
			...
			$content['settings'] = array(
				...
				'description'      => esc_html__( 'Configure default settings.', 'skilled' ),
				'pending'          => esc_html__( 'Pending.', 'skilled' ),
				'installing'       => esc_html__( 'Installing Default Settings.', 'skilled' ),
				'success'          => esc_html__( 'Success.', 'skilled' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Default Content', 'skilled' ); ?></h1>
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'skilled' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'skilled' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				<table class="envato-setup-pages" cellspacing="0">
					<thead>
					<tr>
						<td class="check"></td>
						<th class="item"><?php esc_html_e( 'Item', 'skilled' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'skilled' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'skilled' ); ?></th>
					</tr>
					</thead>
					<tbody>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_content"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'skilled' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'skilled' ) ) );
			}
					...
					if ( ! is_wp_error( $post_id ) ) {
						...
						if ( ! empty( $post_data['meta'] ) ) {
							foreach ( $post_data['meta'] as $meta_key => $meta_val ) {
								...
								// Sensei
								if ( $meta_key == '_course_woocommerce_product' ) {
									$inserted_id = $this->_imported_post_id( intval( $meta_val ) );
									if ( $inserted_id ) {
										$meta_val = $inserted_id;
									}
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'skilled' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'skilled' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'skilled' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'skilled' ), size_format( $max_size ) ) );
				}
			}
		...
		public function _content_install_settings() {
			...
			update_option( SKILLED_THEME_OPTION_NAME, $redux_options );
			update_option( SKILLED_THEME_OPTION_NAME . '-transients', array( 'run_compiler' => 1 ) );
		...
		public function _get_json( $file ) {
			...
			$temp_path = get_transient( 'wheels_temp_path' );
			if ( $temp_path && is_file( $temp_path . basename( $file ) ) ) {
				WP_Filesystem();
				global $wp_filesystem;
				$file_name = $temp_path . basename( $file );
				if ( file_exists( $file_name ) ) {
		...
		public function envato_setup_color_style() {
			...
			?>
			<h1><?php esc_html_e( 'Site Style', 'skilled' ); ?></h1>
			<form method="post">
				<p><?php esc_html_e( 'Please choose your site style below.', 'skilled' ); ?></p>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'skilled' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						?>
						<p><?php esc_html_e( 'Thanks! Theme updates have been enabled for the following items:', 'skilled' ); ?></p>
						<p><?php esc_html_e( 'Theme Updates', 'skilled' ); ?></p>
						<ul>
							<?php foreach ( $my_items as $item ) { ?>
								<li><?php echo esc_html( $item['name'] ); ?></li>
							<?php } ?>
						</ul>
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.', 'skilled' ); ?></p>
						<p><?php esc_html_e( "Change settings from the 'Envato Market' menu in the WordPress Dashboard.", 'skilled' ); ?></p>

						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
						</p>
					<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'skilled' ); ?></p>
						<p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.', 'skilled' ); ?></p>
						<p>
							<em><?php printf( esc_html__( 'On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please %s.', 'skilled' ),
								sprintf( '<a href="%s" target="_blank">%s</a>',
									esc_url( 'https://themeforest.net/user/aislin' ),
									esc_html__( 'contact us', 'skilled' ) ) ); ?></em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'skilled' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'skilled' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
			...
			<?php } else { ?>
				<?php esc_html_e( 'Please ensure the Envato Market plugin has been installed correctly.', 'skilled' ); ?>
				<a href="<?php echo esc_url( $this->get_step_link( 'default_plugins' ) ); ?>"><?php esc_html_e( 'Return to Required Plugins installer', 'skilled' ); ?></a>
					<p class="envato-setup-actions step">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'skilled' ); ?></a>
					</p>
		...
		public function envato_setup_customize() {
			...
			<h1><?php esc_html_e( 'Theme Customization', 'skilled' ); ?></h1>
			<p><?php esc_html_e( 'Most changes to the website can be made through the Theme Options dashboard.', 'skilled' ); ?></p>
			<p><?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars.', 'skilled' ); ?></p>
			<p>
				<em><?php printf( esc_html__( 'Advanced Users: If you are going to make changes to the theme source code please use a %s rather than modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without overwriting your source code changes.', 'skilled' ),
					sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'https://codex.wordpress.org/Child_Themes' ),
					esc_html__( 'Child Theme', 'skilled' ) ) ); ?>
					<br>
					<?php printf( esc_html__( 'See %s in the main folder for a sample.', 'skilled' ),
					sprintf( '<code>%s</code>',
					esc_html__( 'child-theme.zip', 'skilled' ) ) ) ?>
					</em>
			</p>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'skilled' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			?>
			<h1><?php esc_html_e( 'Help and Support', 'skilled' ); ?></h1>
			<p><?php esc_html_e( 'This theme comes with 6 months item support from purchase date (with the option to extend this period). This license allows you to use this theme on a single website. Please purchase an additional license to use this theme on another website.', 'skilled' ); ?></p>
			<p>
				<?php $tf_profile_url = sprintf( '<a href="%1$s" target="_blank">%1$s</a>',
					esc_url( 'https://themeforest.net/user/aislin' ) ); ?>
				<?php printf( esc_html__( 'Item Support can be accessed from %s and includes:', 'skilled'), $tf_profile_url ); ?>
			</p>
			<ul>
				<li><?php esc_html_e( 'Availability of the author to answer questions', 'skilled' ); ?></li>
				<li><?php esc_html_e( 'Answering technical questions about item features', 'skilled' ); ?></li>
				<li><?php esc_html_e( 'Assistance with reported bugs and issues', 'skilled' ); ?></li>
				<li><?php esc_html_e( 'Help with bundled 3rd party plugins', 'skilled' ); ?></li>
			</ul>

			<p><?php printf( esc_html__( 'Item Support %s include:', 'skilled' ), sprintf( '<strong>%s</strong>', esc_html__( 'DOES NOT', 'skilled') ) ); ?></p>
			<ul>
				<li><?php esc_html_e( 'Installation services', 'skilled' ); ?></li>
				<li><?php esc_html_e( 'Customization services', 'skilled' ); ?></li>
				<li><?php esc_html_e( 'Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)', 'skilled' ); ?></li>
			</ul>
			<p><?php $tf_support_policy_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'http://themeforest.net/page/item_support_policy' ),
					esc_html__( 'Item Support Policy', 'skilled' ) ); ?>
				<?php printf( esc_html__( 'More details about item support can be found in the ThemeForest %s.', 'skilled'), $tf_support_policy_url ); ?></p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'skilled' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
		<?php
		}
		...
		public function envato_setup_ready() {
			...
			<a href="<?php echo esc_url( 'https://twitter.com/share' ); ?>" class="twitter-share-button"
			   ...
			   data-via="EnvatoMarket" data-size="large"><?php esc_html_e( 'Tweet', 'skilled' ); ?></a>
			<?php 
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'skilled' ); ?></h1>

			<p><?php esc_html_e( 'Congratulations! The theme has been activated and your website is ready. Login to your WordPress
				dashboard to make changes and modify any of the default content to suit your needs.', 'skilled'); ?></p>
			<p><?php $tf_downloads_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'http://themeforest.net/downloads' ),
					esc_html__( 'leave a 5-star rating', 'skilled' ) ); ?>
				<?php printf( esc_html__( 'Please come back and %s if you are happy with this theme.', 'skilled'), $tf_downloads_url ); ?></p>
			<p><?php $tweeter_url = sprintf( '<a href="%s" target="_blank">%s</a>',
					esc_url( 'https://twitter.com/aislinthemes' ),
					esc_html( '@aislinthemes' ) ); ?>
				<?php printf( esc_html__( 'Follow %s on Twitter to see updates. Thanks!', 'skilled'), $tweeter_url ); ?></p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'skilled' ); ?></h2>
					<ul>
						<li class="setup-product">
						<a class="button button-primary button-large"
                            href="https://twitter.com/aislinthemes"
                            target="_blank">
                            <?php esc_html_e( 'Follow @aislinthemes on Twitter', 'skilled' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'skilled' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'skilled' ); ?></h2>
					<ul>
						<li class="howto"><a href="https://wordpress.org/support/"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'skilled' ); ?></a>
						</li>
						<li class="rating"><a href="http://themeforest.net/downloads"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'skilled' ); ?></a></li>
						<li class="support"><a href="https://themeforest.net/user/aislin"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'skilled' ); ?></a></li>
					</ul>
				</div>
			</div>
		<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class         = 'error';
					echo "<div class=\"$class\"><p>" . sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'skilled' ), $error_message ) . '</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'skilled' ), $this->envato_username ),
			...
			add_settings_field(
				...
				esc_html__( 'oAuth Login', 'skilled' ),
				...
				if ( ! $option || empty( $option['oauth'] ) || empty( $option['oauth'][ $this->envato_username ] ) || empty( $envato_items ) || empty( $envato_items['items'] ) || ! envato_market()->items()->themes( 'purchased' ) ) {
					...
					if ( ! $dissmissed_time || $dissmissed_time < strtotime( '-7 days' ) ) {
						...
						<div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
							<p><?php
								esc_html_e( 'Please activate ThemeForest updates to ensure you have the latest version of this theme.', 'skilled' );
								?></p>

							<p>
								<a class="button button-primary" href="<?php echo esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '' ) ) ) ?>"><?php echo esc_html_e( 'Activate Updates', 'skilled' ); ?></a>
							</p>
						</div>
						<?php 
		...
		public function render_oauth_login_description_callback() {
			printf( esc_html__( 'If you have purchased items from %s on ThemeForest or CodeCanyon please login here for quick and easy updates.', 'skilled' ), esc_html( $this->envato_username ) );
		}
		...
		public function render_oauth_login_fields_callback() {
			...
			<div class="oauth-login" data-username="<?php echo esc_attr( $this->envato_username ); ?>">
				<a href="<?php echo esc_url( $this->get_oauth_login_url( admin_url( 'admin.php?page=' . envato_market()->get_slug() . '#settings' ) ) ); ?>"
				   class="oauth-login-button button button-primary"><?php esc_html_e( 'Login with Envato to activate updates', 'skilled' ); ?></a>
			</div>
		<?php
		}
